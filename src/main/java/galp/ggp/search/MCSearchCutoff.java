...
public class MCSearchCutoff {
	StateMachine propNetStateMachine;
	int nOfSimulations;
	public boolean useCutoff = false;
	int depth = 0, cuted = 0;
	int max = 0, min = 10000000;

	public MCSearchCutoff(StateMachine propNetStateMachine) {
	...
	public void mcSearch(Node root, long timeout)
		...
		while (System.currentTimeMillis() + 100 <= timeout) {
			...
			// CUTOFF: enabling cutoff.
			if (nOfSimulations % 500 == 0 && !useCutoff) {
				if (max - min > 20) {
					useCutoff = true;
					System.out.println("cutoff enabled");
				}
			}
	...
	public List<Integer> runSimulation(MachineState state, long timeout)
		...
		int outOfTreeDepth = 0;
		while (!propNetStateMachine.isTerminal(state)) {

			// CUTOFF: ready to be cut ?
			if (cutoff(outOfTreeDepth)) {
				List<Integer> ret = new ArrayList<Integer>();
				// CUTOFF: creating list for return.
				for (Role r : propNetStateMachine.getRoles()) {
					// CUTOFF: its zero right now, later we can use Frímanns heuristics.
					ret.add(0);
				}
				return ret;
			}
		...
		// CUTOFF: gathering data for cutoff
		if (!useCutoff) {
			int value = (outOfTreeDepth + depth);
			if (max < value)
				max = value;
			if (min > value)
				min = value;
	...
	// CUTOFF: cut or not to cut
	private boolean cutoff(int depth) {
		if (!useCutoff)
			return false;

		if (depth < min + (max - min) / 3)
			return false;
